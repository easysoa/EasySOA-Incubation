...
public class DashboardMatchingSamples {
	...
	public void run() throws Exception {
		...
		registryApi.type(MediaType.APPLICATION_JSON_TYPE).post(
				new SoaNodeResult(new SoaNodeId(Platform.DOCTYPE, "Platform 1"),
				properties, null));
		...
		registryApi.type(MediaType.APPLICATION_JSON_TYPE).post(
				new SoaNodeResult(new SoaNodeId(Platform.DOCTYPE, "Platform 2"),
				properties, null));
		...
		for (int i = 0; i < 5; i++) {
			...
			registryApi.type(MediaType.APPLICATION_JSON_TYPE).post(new SoaNodeResult(new SoaNodeId(InformationService.DOCTYPE, 
		...
		registryApi.type(MediaType.APPLICATION_JSON_TYPE).post(new SoaNodeResult(
		...
		for (int i = 3; i < 6; i++) {
			...
			registryApi.type(MediaType.APPLICATION_JSON_TYPE).post(new SoaNodeResult(new SoaNodeId(ServiceImplementation.DOCTYPE, 
		...
		properties.put(Component.XPATH_LINKED_INFORMATION_SERVICE, getUuid(
				...
				new InformationServiceId(ServiceIdentifierType.WEB_SERVICE, "namespace", "itf0").toString())
			);
		registryApi.type(MediaType.APPLICATION_JSON_TYPE).post(
				new SoaNodeResult(new SoaNodeId(Component.DOCTYPE, "My component"),
				properties, null));
		logger.info("My component");
	}
	
	private String getUuid(String doctype, String soaName) throws Exception {
		SoaNodeResult soaNodeInformation = registryApi.path(InformationService.DOCTYPE)
			.path(doctype)
			.path(soaName)
